...
def tests_get_api_key_for_valid_user(email=valid_email, password=valid_password):
    print(f'Тест 1: Проверяет адекватность  аутентификации пользователя (проверка доступа) ')
    pf = PetFriends()  # Создаем объект для работы с API - экземпляр класса PetFriends
...
def tests_get_all_pets_with_valid_key(filter=''):
    print(f'Теста 2: Получает список питомцев, используя полученный ключ авторизации.  '
          f'Проверяет статус ответа и наличие питомцев ')
...
def tests_post_add_new_pet(name="TestPet", animal_type="Cat", age="1"):
    print(f'Теста 3: Проверяет размещение нового питомца без фотографии')
    ...
    if status == 200:
        print(f"Тест 3 - Status {status}. Питомец/Питомцы размещены")
    else:
...
def test_successful_delete_self_pet(pytest=None):
    print(f'Тест 4: Проверяет удаление питомца/питомцев')
    # Создаем экземпляр класса
    pf = PetFriends()
    """Проверяем возможность удаления питомца"""
    # Получаем ключ auth_key и запрашиваем список СВОИХ питомцев
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    _, my_pets = pf.get_list_of_pets(auth_key['key'], "my_pets")
...
def test_successful_update_self_pet_info(name='Мурзик', animal_type='Котэ', age='1'):
    ...
    print(f'Тест 5: Проверяет возможность обновления информации о питомце')
    # Создаем экземпляр класса
    pf = PetFriends()
    ...
    if len(my_pets['pets']) > 0:
        ...
        if status == 200:
           print(f"Добавление питомцев работает - Status {status}")
           print(f'Добавлено: ({result['name']})')
    else:
